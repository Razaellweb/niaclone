...
const testimonials = [
  {
    ...
    company: "Seedly",
    testimonial:
      "NiaClone helped us discover and evaluate SaaS tools 10x faster. The AI recommendations are spot on—it's like having a YC partner for product research.",
  ...
  },
  {
    ...
    company: "Launchly",
    testimonial:
      "I love how NiaClone personalizes suggestions for my workflow. The integrations with Slack and Notion save our team hours every week.",
    avatar: "https://randomuser.me/api/portraits/women/12.jpg",
  },
  {
    id: 3,
    name: "Chris Wong",
    designation: "Tech Early Adopter",
    company: "BetaList",
    testimonial:
      "Deep product profiles and instant comparisons make NiaClone my go-to for SaaS discovery. It's the YC-quality platform I've been waiting for.",
  ...
  },
  {
    ...
    company: "StackPilot",
    testimonial:
      "The automation features are a game changer. We now evaluate and shortlist tools directly from Slack—no more endless spreadsheets!",
          ...
          {Array.from({ length: count }).map((_, index) => (
            <button
              ...
              className={cn("h-3.5 w-3.5 rounded-full border-2", {
                "bg-accent border-accent": current === index + 1,
              })}
            />
          ))}
        </div>
...
const TestimonialCard = ({
  ...
  <div className="mb-8 bg-accent/10 rounded-xl py-8 px-6 sm:py-6">
    <div className="flex items-center justify-between gap-20">
      <div className="hidden lg:block relative shrink-0 aspect-[3/4] max-w-[18rem] w-full bg-muted-foreground/20 rounded-xl">
        ...
        <div className="absolute top-1/4 right-0 translate-x-1/2 h-12 w-12 bg-accent rounded-full flex items-center justify-center">
          <svg
            width="102"
            height="102"
            viewBox="0 0 102 102"
        ...
        <div className="flex items-center justify-between gap-1">
          <div className="hidden sm:flex md:hidden items-center gap-4">
            <Avatar className="w-8 h-8 md:w-10 md:h-10">
              <AvatarFallback className="text-xl font-medium bg-accent text-accent-foreground">
          ...
          <div className="flex items-center gap-1">
            <StarIcon className="w-5 h-5 fill-accent stroke-accent" />
            <StarIcon className="w-5 h-5 fill-accent stroke-accent" />
            <StarIcon className="w-5 h-5 fill-accent stroke-accent" />
            <StarIcon className="w-5 h-5 fill-accent stroke-accent" />
            <StarIcon className="w-5 h-5 fill-accent stroke-accent" />
